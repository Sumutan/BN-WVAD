...
class XDVideo(data.DataLoader):
    def __init__(self, root_dir, mode, num_segments, len_feature, seed=-1, is_normal=None,list_dir="list"):
        if seed >= 0:
            utils.set_seed(seed)
        ...
        split_path = os.path.join(list_dir,'XD_{}.list'.format(self.mode))
...
class UCFVideo(data.DataLoader):
    def __init__(self, root_dir, mode, num_segments, len_feature, seed=-1, is_normal=None,list_dir="list"):
        ...
        self.feature_path = self.data_path
        split_path = os.path.join(list_dir, 'UCF_{}.list'.format(self.mode))
        split_file = open(split_path, 'r', encoding="utf-8")
        self.vid_list = []
        for line in split_file:
            self.vid_list.append(line.split())
